...
- [x] **阶段五：打字比对核心引擎（最关键模块）**
    - [x] 实现按键事件标准化处理
        - [x] 定义 `IKeyInputEvent` 的具体使用流程（区分原始按键 vs IME 上屏事件）
        - [x] 编写按键事件到内部状态的转换逻辑
    - [x] 实现逐字比对状态机
        - [x] 当前字指针管理（已完成/当前/未到）
        - [x] 字符匹配判断（正确/错误标记）
        - [x] 错误后允许退格重打的状态回退逻辑
    - [x] 实现退格逐字区分逻辑
        - [x] 区分"IME 组字内部退格"（不影响统计）与"已上屏字符退格"（计入退格统计）
    - [x] 编写完整单元测试
        - [x] 模拟正常打字流
        - [x] 模拟错误重打流
        - [x] 模拟中文输入法组字流（拼音输入多按键对应一字）
...
- [x] **阶段七：WPF 前端 —— 基础窗口与导航**
    - [x] 搭建主窗口框架与页面导航结构（MVVM 模式，引入 CommunityToolkit.Mvvm）
    - [x] 实现文章库页面
        - [x] 文章列表展示
        - [x] 导入文章按钮（文件选择/粘贴导入）
        - [x] 文章搜索与标签筛选
    - [x] 实现设置页面框架（主题、字体、快捷键，后续填充）

- [x] **阶段八：WPF 前端 —— 键盘与 IME 事件捕获**
    - [x] 实现 Win32 消息钩子（`HwndSource.AddHook`）捕获 `WM_KEYDOWN`
    - [x] 实现 `WM_IME_CHAR` / `TextInput` 事件捕获上屏内容
    - [x] 将 WPF 原始事件转换为 Core 库的 `IKeyInputEvent` 并喂入 `ITypingSession`
    - [x] 处理特殊键（Backspace、Esc、方向键等的界面响应）
    - [x] 针对常见输入法（微软拼音、搜狗、RIME）进行组字场景实测调试

- [x] **阶段九：WPF 前端 —— 布局 A（逐字交错对齐渲染）**
    - [x] 实现自定义渲染控件（继承 `FrameworkElement`，重写 `OnRender`）
    - [x] 实现字符等宽单元格布局算法（全角/半角字符宽度归一化）
    - [x] 实现原文行与输入行的逐字对齐绘制
    - [x] 实现当前字符高亮、错误字符标红、已完成字符样式
    - [x] 性能优化（避免每次按键全量重绘，采用局部 InvalidateVisual）

- [x] **阶段十：WPF 前端 —— 布局 B（上下分栏跟随渲染）**
    - [x] 实现原文区域的分段染色渲染（已过/当前/未到/错误 四态）
    - [x] 实现输入区域自由输入展示
    - [x] 实现随打字进度自动滚动/翻页逻辑
    - [x] 布局切换功能（用户可在 A/B 布局间切换，共享同一比对引擎状态）

- [x] **阶段十一：WPF 前端 —— 实时统计与结果展示**
    - [x] 练习过程中的实时数据面板（键速、字速、码长、错误数）
    - [x] 练习结束后的结果汇总页面
    - [x] 历史记录页面（按文章查看历次练习记录列表）
    - [x] 历史趋势图表（集成 LiveCharts2，展示速度/错误率随时间变化曲线）

- [x] **阶段十二：码表导入与编码提示功能**
    - [x] 实现码表文件解析器（支持常见码表格式，如极点/小鹤格式的文本码表）
    - [x] 实现码表加载与内存索引构建（`ICodeTableProvider` 实现）
    - [x] 实现打字过程中的编码提示逻辑
        - [x] 根据当前及后续字符反查候选编码
        - [x] 提示浮窗 UI（显示候选编码与说明）
    - [x] 码表管理页面（导入、切换、删除码表）
...
- [x] **阶段十三：设置与个性化功能完善**
    - [x] 主题切换（明暗模式）
    - [x] 字体与字号自定义
    - [x] 快捷键自定义（如暂停/重来/切换布局）
    - [x] 用户偏好持久化（写入本地配置文件或 SQLite）

- [x] **阶段十四：整体测试与优化**
    - [x] Core 库单元测试补全与覆盖率检查
